...
题目文件包含题面、公开样例、资源限制、标签、发布意图、测试集内容版本、checker 和隐藏用例文件。`checksum` 可选；提供时必须等于 `sha256(input + NUL + output)`。Bootstrap 始终重新计算 checksum，对 input/output 分别计算 SHA-256，并生成只保存在数据库内部的内容寻址对象键。
...
- 题面变化只增加题面版本，不无故新建测试集；题单映射整组比较并用唯一约束防重；`today` 使用 manifest 与服务端一致的 `CONTENT_TIMEZONE`。
