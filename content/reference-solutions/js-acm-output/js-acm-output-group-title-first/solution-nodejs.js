...
let output = '';
const t = Number(lines[0] ?? 0); output = lines.slice(1, t + 1).flatMap((line, i) => [`Group ${i + 1}`, String(line.trim().split(/\s+/).filter(Boolean).reduce((sum, value) => sum + Number(value), 0))]).join('\n');
