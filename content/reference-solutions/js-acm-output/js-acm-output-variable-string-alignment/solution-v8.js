...
let output = '';
const n = Number(lines[0] ?? 0); const values = lines.slice(1, n + 1); const width = Math.max(0, ...values.map(value => Array.from(value).length)); output = values.map(value => `${value}${' '.repeat(width - Array.from(value).length)}: ${Array.from(value).length}`).join('\n');
print(String(output));
