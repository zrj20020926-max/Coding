...
let output = '';
const [n] = (lines[0] ?? '0').trim().split(/\s+/).map(Number); const rows = lines.slice(1, n + 1).map(line => line.trim().split(/\s+/)); const width = Math.max(1, ...rows.flat().map(value => value.length)); output = rows.map(row => row.map(value => value.padStart(width, ' ')).join(' ')).join('\n');
print(String(output));
