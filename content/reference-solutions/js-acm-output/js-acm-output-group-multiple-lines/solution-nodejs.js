...
let output = '';
const t = Number(lines[0] ?? 0); output = lines.slice(1, t + 1).flatMap(line => { const values = line.trim().split(/\s+/).filter(Boolean).map(Number); return [String(values.reduce((a, b) => a + b, 0)), String(values.length)]; }).join('\n');
