...
let output = '';
const numberText = tokens[0] ?? '0'; const width = Number(tokens[1] ?? 0); const sign = numberText.startsWith('-') ? '-' : ''; output = sign + numberText.replace(/^[+-]/, '').padStart(Math.max(0, width - sign.length), '0');
